...
This example shows how to create a dynamically provisioned volume created through [Amazon S3 Files access points](https://docs.aws.amazon.com/s3files/latest/ug/s3files-access-points.html) and a persistent volume claim (PVC) that's consumed by a Pod.
...
1. Create a storage class for Amazon S3 Files.
   ...
   2. Download a `StorageClass` manifest for Amazon S3 Files.

      ```sh
      curl -O https://raw.githubusercontent.com/kubernetes-sigs/aws-efs-csi-driver/master/examples/kubernetes/s3files/dynamic_provisioning/specs/storageclass.yaml
      ```
   ...
   3. Edit [the file](./specs/storageclass.yaml). Find the following line, and replace the value for `fileSystemId` with your file system ID.
      ...
      * `provisioningMode` - The type of volume to be provisioned by Amazon S3 Files. Currently, only access point based provisioning is supported (`s3files-ap`).
...
2. Test automatic provisioning by deploying a Pod that makes use of the PVC: 
   ...
   1. Download a manifest that deploys a Pod and a PVC.
      ...
      ```sh
      curl -O https://raw.githubusercontent.com/kubernetes-sigs/aws-efs-csi-driver/master/examples/kubernetes/s3files/dynamic_provisioning/specs/pod.yaml
      ```
...
4. After few seconds, you can observe the controller picking up the change \(edited for readability\). Replace `74ccf9f566-q5989` with a value from one of the Pods in your output from the previous command.
   ...
   ```
   [...]
   1 controller.go:737] successfully created PV pvc-5983ffec-96cf-40c1-9cd6-e5686ca84eca for PVC s3files-claim and csi volume name s3files:fs-95bcec92::fsap-02a88145b865d3a87
   ```
...
5. Confirm that a persistent volume was created with a status of `Bound` to a `PersistentVolumeClaim`:
   ...
   pvc-5983ffec-96cf-40c1-9cd6-e5686ca84eca   20Gi       RWX            Delete           Bound    default/s3files-claim   s3files-sc                  7m57s
   ```
...
6. View details about the `PersistentVolumeClaim` that was created.
   ...
   s3files-claim   Bound    pvc-5983ffec-96cf-40c1-9cd6-e5686ca84eca   20Gi       RWX            s3files-sc         9m7s
   ```
...
**Note**  
When you want to delete an access point in a file system when deleting PVC, you should specify `s3files:ClientRootAccess` to the file system access policy to provide the root permissions. 
