...
| debug-logs                  |        | false   | true     | Set klog verbosity to level 5 and enable debug logging in efs-utils.                                                                                                                                                                    |
...
| efs-cloudwatch-log-enabled   |        |  false      | true     | Enable CloudWatch logging for EFS in efs-utils.conf.                                                                                                                                                                 |
| s3files-cloudwatch-log-enabled   |        |  true      | true     | Enable CloudWatch logging for S3Files in s3files-utils.conf.                                                                                                                                                                 |
| s3files-cloudwatch-metrics-enabled   |        |  true      | true     | Enable CloudWatch metrics emission for S3 files in s3files-utils.conf.                                                                                                                                                                 |
| efs-utils-conf-overrides             |        |  ""        | true     | Comma-separated `section:key=value` pairs to override settings in `efs-utils.conf`. For example, `--efs-utils-conf-overrides=mount-watchdog:stunnel_health_check_interval_min=1`. If a key already exists in the specified section it is updated in place; otherwise it is appended to that section. These overrides take precedence over other flags that control the same config (e.g., `efs-cloudwatch-log-enabled`). Invalid entries (missing `:` or `=`) cause the driver to return an error on startup. For security, the transport-security keys `dns_name_suffix`, `dns_name_format`, `stunnel_cafile`, `stunnel_check_cert_hostname`, and `stunnel_check_cert_validity` are denied (case-insensitive) and cause the driver to return an error on startup. |
| s3files-utils-conf-overrides         |        |  ""        | true     | Comma-separated `section:key=value` pairs to override settings in `s3files-utils.conf`. Follows the same format and validation rules as `efs-utils-conf-overrides`. These overrides take precedence over other flags that control the same config (e.g., `s3files-cloudwatch-log-enabled`, `s3files-cloudwatch-metrics-enabled`). For security, the transport-security keys `dns_name_suffix`, `dns_name_format`, `stunnel_cafile`, `stunnel_check_cert_hostname`, and `stunnel_check_cert_validity` are denied (case-insensitive) and cause the driver to return an error on startup. |
...
| debug-logs                  |        | false  | true     | Set klog verbosity to level 5 and enable debug logging in efs-utils.                                                                                                                                                                    |
| tags                         |       |         | true     | Space separated key:value pairs which will be added as tags for Amazon EFS resources. For example, '--tags=name:efs-tag-test date:Jan24'. To include a ':' or ' ' in the tag name or value, use \ as an escape character, for example '--tags="tag\:name:tag\:value". **NOTE:** Custom tags require the controller's IAM policy to allow those tag keys on `elasticfilesystem:CreateAccessPoint`. The AWS-managed `AmazonEFSCSIDriverPolicy` only permits the default `efs.csi.aws.com/cluster` tag key, so custom tags fail access point creation (PVCs stuck in `Pending`) unless a custom policy is attached. See [Custom tags require additional IAM permissions](./install.md#custom-tags-require-additional-iam-permissions). |
