...
| extraTags        |        |    ""     | true     | Allows a set of extra AWS tags to be added to the Access Point when it is provisioned. Be aware, if one of the tags has the same key as another specified in `.Values.controller.tags` this value will override it. The syntax is exactly the same as for `.Values.controller.tags` |
