...
func (a *AccessPointProvisioner) Provision(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.Volume, error) {
	...
	localCloud, roleArn, crossAccountDNSEnabled, err = getCloud(req.GetSecrets(), a.cloud, a.adaptiveRetryMode)
	if err != nil {
		return nil, err
	}
	...
	if reuseAccessPoint {
		...
		if existingAP != nil {
			...
			if a.lockManager.lockMutex(accessPoint.AccessPointId, ApLockWaitTimeSec*time.Second) {
				defer a.lockManager.unlockMutex(accessPoint.AccessPointId)
			} else {
...
func (a *AccessPointProvisioner) Delete(ctx context.Context, req *csi.DeleteVolumeRequest) error {
	...
	fileSystemId, _, accessPointId, _ := parseVolumeId(volId)

	// Lock on the access point ID to ensure a retry won't race with the in-progress deletion
	if a.lockManager.lockMutex(accessPointId, ApLockWaitTimeSec*time.Second) {
		defer a.lockManager.unlockMutex(accessPointId)
	} else {
		return status.Errorf(codes.Internal, "Could not take the lock to delete access point: %v", accessPointId)
	}
	...
	if a.deleteAccessPointRootDir {
		...
		defer func() {
			if !deleteCompleted {
				...
				isNotMounted, err := a.mounter.IsLikelyNotMountPoint(fsRoot)
				if err != nil {
					return // Skip cleanup, we can't verify mount status
				}

				if !isNotMounted {
					if err := a.mounter.Unmount(fsRoot); err != nil {
		...
		if err := a.mounter.MakeDir(fsRoot); err != nil {
			return status.Errorf(codes.Internal, "Could not create dir %q: %v", fsRoot, err)
		}

		// Only attempt to mount the target filesystem if its not already mounted
		isNotMounted, err := a.mounter.IsLikelyNotMountPoint(fsRoot)
		if err != nil {
			return status.Errorf(codes.Internal, "Could not check if %q is mounted: %v", fsRoot, err)
		}
		if isNotMounted {
			if err := a.mounter.Mount(fileSystemId, fsRoot, "efs", mountOptions); err != nil {
				return status.Errorf(codes.Internal, "Could not mount %q at %q: %v", fileSystemId, fsRoot, err)
			}
		}

		// Before removing, ensure the removal path exists and is a directory
		apRootPath := fsRoot + accessPoint.AccessPointRootDir
		if pathInfo, err := a.mounter.Stat(apRootPath); err == nil && !os.IsNotExist(err) && pathInfo.IsDir() {
		...
		err = a.mounter.Unmount(fsRoot)
		if err != nil {
			return status.Errorf(codes.Internal, "Could not unmount %q: %v", fsRoot, err)
		}
		err = os.Remove(fsRoot)
		if err != nil && !os.IsNotExist(err) {
			return status.Errorf(codes.Internal, "Could not delete %q: %v", fsRoot, err)
		}
