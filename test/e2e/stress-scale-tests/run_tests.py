...
def main():
    ...
    try:
        ...
        if args.test_suite in ['chaos', 'all']:
            logger.info("Running chaos tests")
            
            if args.dry_run:
                logger.info("DRY RUN MODE: Would run chaos tests to stress the CSI driver")
                chaos_results = {
                    'api_flood': {'status': 'would_run'},
                    'race_conditions': {'status': 'would_run'},
                    'resource_exhaustion': {'status': 'would_run'},
                    'mixed_chaos': {'status': 'would_run'}
                }
            else:
                # Import here to avoid loading it unnecessarily for other tests
                from tests.chaos_test import ChaosTester
                
                logger.warning("CAUTION: Running chaos tests that may cause system instability")
                chaos_tester = ChaosTester(args.config)
                chaos_results = chaos_tester.run_all_tests()
                
                # Generate chaos test specific reports
                chaos_report_dir = os.path.join(report_dir, 'chaos')
                os.makedirs(chaos_report_dir, exist_ok=True)
                
                # Write a dedicated report for chaos tests
                chaos_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                chaos_report_path = os.path.join(chaos_report_dir, f"efs_chaos_{chaos_timestamp}.json")
                with open(chaos_report_path, 'w') as f:
                    import json
                    json.dump(chaos_results, f, indent=2)
                logger.info(f"Chaos test report generated: {chaos_report_path}")
            
            results['chaos'] = chaos_results
        
        # Collect CSI metrics
        metrics_collector.collect_csi_metrics(config)
        results['metrics'] = metrics_collector.get_all_metrics()
        
        # Generate reports
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_formats = config.get('reporting', {}).get('formats', ['json'])
        
        if 'json' in report_formats:
            json_path = report_generator.generate_json_report(results, f"efs_tests_{timestamp}")
            logger.info(f"JSON report generated: {json_path}")
            
        if 'summary' in report_formats:
            summary_path = report_generator.generate_summary_report(results, f"efs_tests_{timestamp}")
            logger.info(f"Summary report generated: {summary_path}")
        
        logger.info("All tests completed successfully")
        
        # Clean up resources if cluster was set up and --skip-cleanup wasn't specified - DISABLED
        if args.setup_cluster and not args.skip_cleanup:
            ...
            try:
                ...
                # Don't exit with failure, as the tests themselves succeeded
            """
        elif args.setup_cluster and args.skip_cleanup:
            logger.warning("Cluster cleanup functionality is disabled")
